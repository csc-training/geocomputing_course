...
* Access to Roihu webinterface
* Own directory within the course directory `/scratch/project_2020458/students/$USER`
...
These examples are done on Roihu. When using the web interface, you can open a compute node shell directly.
...
```bash
srun  --time=00:10:00 --pty --account=project_2020458 --partition=interactive  bash      ##replace xxxxx with your project number; you can also add --reservation=geocomputing_day1 here for the course (not available at other times), change partition to small then
```

**or** on Roihu you can also use the `sinteractive` wrapper to start an interactive session from the **login node**, which simplifies the call and asks you for the resources step by step: 
...
```bash
sinteractive --account project_2020458 --time 00:10:00         # replace xxxxx with your CSC project, e.g. project_2001234
```
...
Examples are done on Roihu. In the Roihu web interface, open a login node shell.
...
```bash
cd /scratch/project_2020458/students/$USER      # replace xxxxx with your CSC project number
```
...
```bash
#!/bin/bash
#SBATCH --account=project_2020458   # Choose the billing project. Has to be defined!
...
hostname                    # Run hostname-command, that will print the name of the Roihu compute node that has been allocated for this particular job
