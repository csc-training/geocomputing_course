...
* Own directory within the course directory `/scratch/project_2015299/students/$USER`
...
```bash
srun  --time=00:10:00 --pty --account=project_2015299 --partition=interactive  bash      ##replace xxxxx with your project number; you can also add --reservation=geocomputing_day1 here for the course (not available at other times), change partition to small then
```
...
```bash
sinteractive --account project_2015299 --time 00:10:00         # replace xxxxx with your CSC project, e.g. project_2001234
```
...
```bash
#!/bin/bash
#SBATCH --account=project_2015299   # Choose the billing project. Has to be defined!
